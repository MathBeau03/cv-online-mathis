...
app.get('/generator', (req, res) => res.sendFile(path.join(__dirname, 'generator', 'index.html')));
...
const uploadLinkedin = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    else cb(new Error('Seuls les fichiers PDF sont acceptés pour LinkedIn.'));
  },
});
...
// Parse LinkedIn PDF
app.post('/api/generator/parse-linkedin', uploadLinkedin.single('linkedin_pdf'), handleMulterError, async (req, res) => {
  const limit = checkLimit(req, 'generator', 5);
  if (!limit.allowed) {
    return res.status(429).json({
      error: 'Limite journalière atteinte (5 utilisations/jour). Revenez demain 🙂',
      limit: true,
    });
  }
  try {
    ...
    if (!text.trim()) {
      return res.status(400).json({ error: 'Impossible de lire ce PDF. Assurez-vous que c\'est bien un export LinkedIn.' });
    }

    const prompt = `Voici le texte extrait d'un profil LinkedIn exporté en PDF. Extrais toutes les informations et retourne UNIQUEMENT un JSON strict (sans markdown) :
{
  "prenom": "",
  "nom": "",
  "email": "",
  "telephone": "",
  "localisation": "",
  "resume_pro": "résumé/accroche du profil",
  "experiences": [{"title":"","company":"","location":"","dates":"","desc":"description des missions"}],
  "education": [{"degree":"","school":"","year":"","details":""}],
  "competences_tech": "liste séparée par virgules",
  "soft_skills": "liste séparée par virgules",
  "langues": "ex: Français (natif), Anglais (C1)",
  "certifications": "liste séparée par virgules"
}
...
${text.substring(0, 8000)}`;
    ...
    if (!apiRes.ok) return res.status(502).json({ error: 'Erreur API Claude.' });
    ...
    try {
      ...
      return res.status(502).json({ error: 'Réponse invalide de l\'IA.' });
    }

    res.json({ profile });
  } catch (err) {
    console.error('LinkedIn parse error:', err);
    res.status(500).json({ error: 'Erreur serveur lors de l\'analyse LinkedIn.' });
  }
});

// Analyze job offer
app.post('/api/generator/analyze', upload.single('file'), handleMulterError, async (req, res) => {
  const limit = checkLimit(req, 'generator', 5);
  if (!limit.allowed) {
    return res.status(429).json({
      error: 'Limite journalière atteinte (5 utilisations/jour). Revenez demain 🙂',
  ...
  try {
    ...
    if (req.file) {
      ...
      if (mime === 'application/pdf') {
        ...
        if (!extractedText.trim()) extractedText = '[PDF sans texte extractible — contenu scanné probable]';
        claudeContent = [{ type: 'text', text: `Voici le texte extrait d'une offre d'emploi en PDF :\n\n${extractedText}` }];
      } else {
      ...
      claudeContent = [{ type: 'text', text: `Voici le texte d'une offre d'emploi :\n\n${extractedText}` }];
    } else {
      return res.status(400).json({ error: 'Aucun fichier ni texte fourni.' });
    }

    const extractPrompt = `Tu es un expert RH. Analyse cette offre d'emploi et extrais les informations clés.
Réponds UNIQUEMENT avec un objet JSON valide, sans balises markdown, sans commentaire :
{
  "poste": "...",
  "entreprise": "...",
  "localisation": "...",
...
Si une information est absente, utilise une chaîne vide ou un tableau vide.`.trim();

    claudeContent.push({ type: 'text', text: extractPrompt });
    ...
    const data = await apiRes.json();
    if (!apiRes.ok) {
      console.error('Anthropic analyze error:', data);
      return res.status(502).json({ error: 'Erreur API Claude lors de l\'analyse.' });
    }
    ...
    res.json({ analysis, rawText: extractedText });
  } catch (err) {
...
app.post('/api/generator/generate', async (req, res) => {
  ...
  try {
    const { jobAnalysis, profile } = req.body;
    if (!jobAnalysis || !profile) {
      return res.status(400).json({ error: 'jobAnalysis et profile sont requis.' });
    }

    const prompt = `Tu es un expert en recrutement et rédaction de CV. Génère un CV et une lettre de motivation personnalisés.

OFFRE D'EMPLOI :
- Poste : ${jobAnalysis.poste || 'Non précisé'}
- Entreprise : ${jobAnalysis.entreprise || 'Non précisée'}
- Localisation : ${jobAnalysis.localisation || 'Non précisée'}
- Type de contrat : ${jobAnalysis.type_contrat || 'Non précisé'}
- Compétences requises : ${(jobAnalysis.competences_requises || []).join(', ')}
- Missions : ${(jobAnalysis.missions || []).join(' | ')}
- Profil recherché : ${jobAnalysis.profil_recherche || ''}
...
- Résumé professionnel : ${profile.resume_pro || ''}
- Compétences techniques : ${profile.competences_tech || ''}
- Soft skills : ${profile.soft_skills || ''}
- Langues : ${profile.langues || ''}
- Certifications : ${profile.certifications || ''}
- Expériences : ${JSON.stringify(profile.experiences || [])}
- Formations : ${JSON.stringify(profile.education || [])}

INSTRUCTIONS :
1. Adapte le CV pour correspondre aux mots-clés et exigences du poste.
2. Rédige la lettre de motivation en français, personnalisée, convaincante, 3-4 paragraphes.
3. Mets en valeur les expériences les plus pertinentes pour ce poste.
4. La lettre doit montrer une vraie motivation pour l'entreprise et le poste.
5. Retourne UNIQUEMENT un JSON strict (pas de markdown, pas de balises \`\`\`) :

{
  "cv": {
    "headline": "Titre professionnel accrocheur adapté au poste",
    "summary": "Résumé professionnel 2-3 phrases percutantes",
  ...
  "letter": "texte complet de la lettre avec \\n pour les sauts de ligne"
    ...
    const data = await apiRes.json();
    if (!apiRes.ok) {
      console.error('Anthropic generate error:', data);
      return res.status(502).json({ error: 'Erreur API Claude lors de la génération.' });
    }
